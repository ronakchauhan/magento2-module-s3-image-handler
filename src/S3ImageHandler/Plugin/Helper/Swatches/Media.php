...
namespace Y1\S3ImageHandler\Plugin\Helper\Swatches;
 ...
 * Class Media
 *
 * @package Y1\S3ImageHandler\Plugin\Helper\Swatches
 */
class Media
{
    /**
     ...
     * @return array
     */
    public function beforeMoveImageFromTmp(\Magento\Swatches\Helper\Media $subject, $file)
    {
    ...
    public function aroundGenerateSwatchVariations(Media $subject, callable $proceed, $imageUrl)
    {
        if ($this->fileStorageDb->checkDbUsage()) {
            ...
            foreach ($this->swatchImageTypes as $swatchType) {
                ...
                $swatchPath = $subject->getSwatchCachePath($swatchType) . $subject->getFolderNameSize(
                        $swatchType,
                        $imageConfig
